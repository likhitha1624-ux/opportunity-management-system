...
// store logged-in email safely
const originalShowDashboardRef = showDashboard;
showDashboard = function(email) {
    currentAdminEmail = email;
    originalShowDashboardRef(email);
    loadOpportunities();
};
...
function loadOpportunities() {
    fetch(`http://127.0.0.1:5000/api/opportunities?email=${currentAdminEmail}`)
    .then(res => res.json())
    .then(data => {
        ...
        data.data.forEach(op => createOpportunityCard(op));
    })
...
function createOpportunityCard(op) {
    ...
    card.innerHTML = `
        ...
        <div class="opportunity-footer">
            <button class="view-course-btn view-btn">View</button>
            <button class="view-course-btn edit-btn">Edit</button>
            <button class="view-course-btn delete-btn">Delete</button>
        </div>
    `;
    ...
    card.querySelector('.edit-btn').addEventListener('click', () => {
        ...
        form.onsubmit = function(e) {
            ...
            fetch(`http://127.0.0.1:5000/api/opportunities/${op.id}`, {
                method: "PUT",
    ...
    card.querySelector('.delete-btn').addEventListener('click', () => {
        ...
        fetch(`http://127.0.0.1:5000/api/opportunities/${op.id}?email=${currentAdminEmail}`, {
            method: "DELETE"
        })
        .then(res => res.json())
        .then(() => {
